...
%next we plot the thrust of the rocket as a function of time
%Because we already got the thrust returned from the RocketODE2 function we
%can simply plot that thrust with the time span of the rocket
figure(2)
plot(t,fatThrust);
ylim([0,300])
xlim([0,.45]) %this time span is smaller than the actual because most of the rocket's flight is ballistic in which there is no thrust
hold on
%We can use the same indices of where the phase changes occur to plot
%points at which there is a phase change
p3 = plot(t(x),fatThrust(x),'.');
p3.MarkerSize = 20;
p4 = plot(t(y),fatThrust(y),'.');
p4.MarkerSize = 20;
xlabel('Time span of flight [s]');
